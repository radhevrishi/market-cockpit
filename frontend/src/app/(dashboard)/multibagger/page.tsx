...
function USAAnalytics() {
  ...
  const stats = React.useMemo(() => {
    ...
    const r40Buckets: Array<{ label: string; n: number; color: string }> = [
      { label: '<0',     n: rows.filter(r => (r.ruleOf40 ?? -999) < 0).length,                                              color: RED },
      { label: '0–20',   n: rows.filter(r => { const x = r.ruleOf40 ?? -999; return x >= 0 && x < 20; }).length,            color: ORANGE },
      { label: '20–40',  n: rows.filter(r => { const x = r.ruleOf40 ?? -999; return x >= 20 && x < 40; }).length,           color: YELLOW },
      { label: '40–60',  n: rows.filter(r => { const x = r.ruleOf40 ?? -999; return x >= 40 && x < 60; }).length,           color: GREEN },
      { label: '60–80',  n: rows.filter(r => { const x = r.ruleOf40 ?? -999; return x >= 60 && x < 80; }).length,           color: ACCENT },
      { label: '≥80',    n: rows.filter(r => (r.ruleOf40 ?? -999) >= 80).length,                                            color: PURPLE },
    ];
    // Piotroski buckets
    ...
    // Leaderboard
    const top15 = [...rows].sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 15);
    return {
  ...
  return (
    <div style={{ maxWidth: 1800, margin: '0 auto', padding: '20px 24px 32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginBottom: 18 }}>
        {[
          { label: 'STOCKS',           value: stats.n.toString(),                                                color: PURPLE },
          { label: 'AVG SCORE',        value: stats.avgScore.toFixed(1),                                        color: ACCENT },
          { label: 'A+/A GRADES',      value: `${stats.topGradeCount} (${stats.n ? (100 * stats.topGradeCount / stats.n).toFixed(0) : 0}%)`, color: GREEN },
          { label: 'R40 ≥40 / ≥60',    value: `${stats.r40Pass} / ${stats.r40Elite}`,                           color: ACCENT },
          { label: 'PIOTROSKI ≥7',     value: stats.piotroskiPass.toString(),                                   color: GREEN },
          { label: 'ALTMAN Z ≥3',      value: stats.altmanSafe.toString(),                                      color: GREEN },
          { label: 'ACCELERATING',     value: stats.acceleratingCount.toString(),                               color: PURPLE },
          { label: 'CONVICTION',       value: stats.convictionOverlap.toString(),                               color: PURPLE },
          { label: 'AGG MCAP $B',      value: stats.totalMcapB >= 1000 ? `${(stats.totalMcapB / 1000).toFixed(1)}T` : stats.totalMcapB.toFixed(0), color: TEXT },
          { label: 'NEW (vs PREV)',    value: stats.newSinceLast.toString(),                                    color: stats.newSinceLast > 0 ? GREEN : MUTED },
        ].map((s) => (
          <div key={s.label} style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '12px 14px' }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: s.color, fontFamily: 'ui-monospace,monospace', marginTop: 2 }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      {/* ── TOP 15 LEADERBOARD ────────────────────────────────────── */}
      <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 16 }}>
        <div style={{ fontSize: F2.sm, fontWeight: 800, color: TEXT, marginBottom: 12 }}>TOP 15 BY SCORE</div>
...
function TurnaroundAnalytics() {
  ...
  const stats = React.useMemo(() => {
    const n = rows.length;
    const avgScore = n ? rows.reduce((s, r) => s + (r.totalScore || 0), 0) / n : 0;
    const buyZone = rows.filter(r => r.inBuyZone).length;
    const best = rows.filter(r => r.isBestCandidate).length;
    const confirmed = rows.filter(r => r.stage === 'CONFIRMED').length;
    const survivalPass = rows.filter(r => (r.survivalScore || 0) >= 6).length;
    const highConcall = rows.filter(r => (r.concallScore || 0) >= 15).length;
    const realTurnarounds = rows.filter(r => r.archetype === 'TURNAROUND').length;
    const valueTraps = rows.filter(r => r.archetype === 'VALUE-TRAP').length;
    // Stage distribution
    ...
    const archetypeBuckets = archetypes.map(a => ({
      label: a,
      n: rows.filter(r => r.archetype === a).length,
      color: a === 'TURNAROUND' ? PURPLE : a === 'GROWTH' ? GREEN : a === 'QUALITY' ? ACCENT : a === 'VALUE-TRAP' ? RED : a === 'DECLINING' ? ORANGE : a === 'WAIT' ? YELLOW : MUTED,
    }));
    // Turnaround type
    const typeBuckets: Array<{ label: string; n: number; color: string }> = [
      { label: 'CYCLICAL',    n: rows.filter(r => r.turnaroundType === 'CYCLICAL').length,    color: ACCENT },
      { label: 'OPERATIONAL', n: rows.filter(r => r.turnaroundType === 'OPERATIONAL').length, color: PURPLE },
      { label: 'DISTRESSED',  n: rows.filter(r => r.turnaroundType === 'DISTRESSED').length,  color: ORANGE },
      { label: 'UNKNOWN',     n: rows.filter(r => r.turnaroundType === 'UNKNOWN' || !r.turnaroundType).length, color: MUTED },
    ];
    // Phase
    const phaseBuckets = ([1, 2, 3, 4] as const).map(p => ({
      label: p === 1 ? '1 · COLLAPSE' : p === 2 ? '2 · STABILISATION' : p === 3 ? '3 · INFLECTION (BUY)' : '4 · RE-RATING',
      n: rows.filter(r => r.phase === p).length,
      color: p === 1 ? RED : p === 2 ? ORANGE : p === 3 ? GREEN : ACCENT,
    }));
    // Survival score buckets
    const survivalBuckets = [
      { label: '0–2 (fragile)', n: rows.filter(r => (r.survivalScore ?? -1) >= 0 && (r.survivalScore ?? -1) <= 2).length, color: RED },
      { label: '3–5 (mid)',     n: rows.filter(r => (r.survivalScore ?? -1) >= 3 && (r.survivalScore ?? -1) <= 5).length, color: YELLOW },
      { label: '6–8 (robust)',  n: rows.filter(r => (r.survivalScore ?? -1) >= 6).length,                                  color: GREEN },
    ];
    // Decision overlay
    const decisionCounts = { BUY: 0, WATCH: 0, REJECTED: 0, NONE: 0 };
    rows.forEach(r => {
    ...
    const convictionOverlap = rows.filter(r => cbUpper.has((r.symbol || '').toUpperCase().trim())).length;
    // Best candidates
    const bestList = [...rows].filter(r => r.isBestCandidate).sort((a, b) => (b.totalScore || 0) - (a.totalScore || 0));
    // Top 12 overall
    const top12 = [...rows].sort((a, b) => (b.totalScore || 0) - (a.totalScore || 0)).slice(0, 12);
    return {
      n, avgScore, buyZone, best, confirmed, survivalPass, highConcall, realTurnarounds, valueTraps,
      stageBuckets, archetypeBuckets, typeBuckets, phaseBuckets, survivalBuckets,
      decisionCounts, convictionOverlap, bestList, top12,
    };
  }, [rows, convictionSet, decisionTick]);
  ...
  return (
    <div style={{ maxWidth: 1800, margin: '0 auto', padding: '20px 24px 32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 18 }}>
        {[
          { label: 'CANDIDATES',     value: stats.n.toString(),                                                                                            color: PURPLE },
          { label: 'AVG SCORE',      value: stats.avgScore.toFixed(1),                                                                                     color: ACCENT },
          { label: 'BUY-ZONE',       value: `${stats.buyZone} (${stats.n ? (100 * stats.buyZone / stats.n).toFixed(0) : 0}%)`,                            color: GREEN },
          { label: 'BEST CAND.',     value: stats.best.toString(),                                                                                         color: GREEN },
          { label: 'CONFIRMED',      value: stats.confirmed.toString(),                                                                                    color: ACCENT },
          { label: 'SURVIVAL ≥6',    value: stats.survivalPass.toString(),                                                                                 color: GREEN },
          { label: 'CONCALL ≥15',    value: stats.highConcall.toString(),                                                                                  color: PURPLE },
          { label: 'REAL T-AROUNDS', value: `${stats.realTurnarounds} / ${stats.n}`,                                                                       color: PURPLE },
          { label: 'VALUE TRAPS',    value: stats.valueTraps.toString(),                                                                                   color: stats.valueTraps > 0 ? RED : MUTED },
          { label: 'CONVICTION',     value: stats.convictionOverlap.toString(),                                                                            color: PURPLE },
        ].map((s) => (
          <div key={s.label} style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '12px 14px' }}>
            <div style={{ fontSize: 10, color: MUTED, fontWeight: 700, letterSpacing: 0.6 }}>{s.label}</div>
