...
export default function EarningsPage() {
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // PATCH 1101zzz50 — dropped BATCH_SIZE 30 → 10. Server-side screener.in
      // fetcher chokes when 30 concurrent symbol enrichments fire (verified:
      // individual UNIPARTS returns FULL data in 2.6s, 29-symbol batch
      // returned MISSING for ALL in 10s). Smaller batches finish cleanly.
      // PARALLEL stays at 3 so total throughput is 30 in-flight (was 90).
      const BATCH_SIZE = 10;
      const PARALLEL = 3;
