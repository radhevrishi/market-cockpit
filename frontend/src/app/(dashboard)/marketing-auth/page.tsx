...
// ═══════════════════════════════════════════════════════════════════════════
// MARKETING AUTHORIZATION TRACKER (TheWrap Module 5)
//
// Surfaces pharma regulatory milestones — USFDA approval/EIR/483, ANDA, MHRA,
// WHO-GMP, CEP, EMA, DCGI, tech-transfer, biosimilar/label-expansion, etc.
// Runs detectMarketingAuth() over the dual-source news + NSE/BSE filings feed
// (lib/thewrap-feed.ts). Each is a distinct pharma re-rating catalyst.
// ═══════════════════════════════════════════════════════════════════════════

import { useEffect, useMemo, useState } from 'react';
import { detectMarketingAuth } from '@/lib/thewrap-detectors';
import {
  fetchWrapFeed,
  runDetector,
  relativeTime,
...
const PAGE_TITLE = 'Marketing Authorization';
const PAGE_EMOJI = '💊';
const PAGE_DESC =
  'Pharma regulatory milestones — USFDA approval / EIR / 483, ANDA, MHRA, WHO-GMP, CEP, EMA, DCGI, tech-transfer, biosimilar & label expansion — each a distinct approval-driven re-rating catalyst.';
const FACET_KEY = 'kind';
const FACET_LABEL = 'Milestone';
const EMPTY_THING = 'marketing authorizations';
const ACCENT = '#10B981';
...
export default function MarketingAuthPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const found = runDetector(res.items, detectMarketingAuth);
      setMatches(found);
    ...
    <div style={{ padding: '20px 24px', maxWidth: 1180, margin: '0 auto', color: '#E2E8F0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 18, flexWrap: 'wrap' }}>
        <input
          ...
          placeholder="Search ticker, company, milestone…"
          style={inputStyle}
        />
