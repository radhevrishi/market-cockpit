...
// zzz403/zzz405 — Nifty Smallcap 100 Scenario Simulator.
//
// India counterpart to /qqq-simulator. Historical base rates + conditional
// cascade from TheWrap's 22-year deck (Nifty Smallcap 100, 2004–2025).
//
// zzz405: nominal + real return on every year card; forward-return haircut;
// not live (edit → Refresh); wider layout + larger type.
...
interface Committed { probs: number[]; capital: number; mode: 'lumpsum' | 'sip'; monthlySip: number; realReturns: boolean; vol: number; haircut: number; }
const DEFAULTS: Committed = { probs: SCENARIOS.map(s => s.defaultPct), capital: 100000, mode: 'lumpsum', monthlySip: 10000, realReturns: false, vol: 42, haircut: 0 };

interface MCResult { band: { p10: number; p25: number; p50: number; p75: number; p90: number }[]; benchBand: { p50: number }[]; ending: { p10: number; p25: number; p50: number; p75: number; p90: number }; cagr: { p10: number; p50: number; p90: number }; pLoss: number; pDouble: number; pHalf: number; invested: number; benchMedianEnd: number; }

export default function SmallcapSimulatorPage() {
  const [probs, setProbs] = useState<number[]>(DEFAULTS.probs);
  ...
  useEffect(() => {
    try {
      ...
      setRealReturns(snap.realReturns); setVol(snap.vol); setHaircut(snap.haircut); setCommitted(snap);
    } catch { /* ignore */ }
  }, []);

  const liveTotal = probs.reduce((a, b) => a + b, 0);
  const live: Committed = { probs, capital, mode, monthlySip, realReturns, vol, haircut };
  const stale = JSON.stringify(live) !== JSON.stringify(committed);
  const run = useCallback(() => setCommitted({ probs: probs.slice(), capital, mode, monthlySip, realReturns, vol, haircut }), [probs, capital, mode, monthlySip, realReturns, vol, haircut]);
  ...
  const { expNom, expReal } = useMemo(() => {
    ...
    const nom = [e2026, eY(2027), eY(2028), eY(2029), eY(2030)].map(r => r - committed.haircut);
    return { expNom: nom, expReal: nom.map(r => r - INFLATION) };
  }, [norm, committed]);
  const expected = committed.realReturns ? expReal : expNom;
  ...
  const wealthPath = useMemo(() => {
    ...
    FORECAST_YEARS.forEach((yr, i) => { const g = 1 + expected[i] / 100; if (committed.mode === 'sip') w = w * g + annual * (1 + expected[i] / 200); else w = w * g; pts.push({ yr: `Dec ${yr}`, val: w }); });
    return pts;
  }, [expected, committed]);
  ...
  const mc = useMemo<MCResult>(() => {
    ...
    for (let s = 0; s < N_SIMS; s++) {
      ...
      ([2027, 2028, 2029, 2030] as const).forEach(y => seq.push(clamp(path[y] + gaussian(rng) * committed.vol, -85, 260)));
      ...
      for (let y = 0; y < 5; y++) {
        const nomRet = seq[y] - committed.haircut; yearRet[y].push(nomRet); benchRet[y].push(bseq[y]);
        const rS = committed.realReturns ? nomRet - INFLATION : nomRet; const rB = committed.realReturns ? bseq[y] - INFLATION : bseq[y];
        const gS = 1 + rS / 100, gB = 1 + rB / 100;
        if (committed.mode === 'sip') { w = w * gS + annual * (1 + rS / 200); wb = wb * gB + annual * (1 + rB / 200); } else { w = w * gS; wb = wb * gB; }
      }
    ...
    return { band, benchBand, ending: { p10: pctile(soEnd, 10), p25: pctile(soEnd, 25), p50: pctile(soEnd, 50), p75: pctile(soEnd, 75), p90: pctile(soEnd, 90) }, cagr: { p10: pctile(cagrArr, 10), p50: pctile(cagrArr, 50), p90: pctile(cagrArr, 90) }, pLoss: (endW.filter(w => w < inv).length / endW.length) * 100, pDouble: (endW.filter(w => w >= inv * 2).length / endW.length) * 100, pHalf: (endW.filter(w => w <= inv * 0.5).length / endW.length) * 100, invested: inv, benchMedianEnd: pctile(soBench, 50) };
  }, [norm, committed]);
  ...
  const copyLink = useCallback(() => {
    try { const qs = new URLSearchParams(); qs.set('p', probs.join(',')); qs.set('cap', String(capital)); if (mode === 'sip') { qs.set('mode', 'sip'); qs.set('sip', String(monthlySip)); } if (realReturns) qs.set('real', '1'); qs.set('vol', String(vol)); if (haircut) qs.set('hc', String(haircut)); navigator.clipboard?.writeText(`${window.location.origin}${window.location.pathname}?${qs.toString()}`); setSavedFlash('🔗 Link copied'); setTimeout(() => setSavedFlash(''), 2000); } catch { setSavedFlash('⚠ Could not copy'); }
  }, [probs, capital, mode, monthlySip, realReturns, vol, haircut]);
  ...
  const RefreshBar = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 16, padding: '12px 16px', background: stale ? `${C.amber}12` : `${C.green}0c`, border: `1px solid ${stale ? C.amber : C.green}44`, borderRadius: 12 }}>
      <button onClick={run} disabled={!stale} style={{ ...chipBtn(false, stale ? C.amber : C.green), fontSize: 13.5, padding: '9px 18px', opacity: stale ? 1 : 0.6, cursor: stale ? 'pointer' : 'default' }}>⟳ {stale ? 'Refresh projections' : 'Up to date'}</button>
      <span style={{ ...mono, fontSize: 12, color: stale ? C.amber : C.text3 }}>{stale ? 'Inputs changed — click Refresh to recompute the projection & Monte-Carlo.' : 'Projection reflects your current inputs.'}</span>
    </div>
  );
      ...
      <div style={{ maxWidth: 1360, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 18, padding: '18px 22px', background: `linear-gradient(135deg, ${C.purple}18 0%, ${C.panel} 100%)`, border: `1px solid ${C.purple}33`, borderRadius: 14, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ ...mono, fontSize: 13, color: C.text3, marginTop: 6 }}>Set 2026 assumptions → cascading 2027–2030 with a Monte-Carlo range. Edit, then Refresh.</div>
          </div>
        ...
        <div style={card}>
          ...
          <div style={{ fontSize: 13.5, color: C.text2, lineHeight: 1.65 }}>
            After the 6 historical negative years, the next year was: <span style={{ color: C.green, fontWeight: 700 }}>+107%</span>, <span style={{ color: C.green, fontWeight: 700 }}>+36.8%</span>, <span style={{ color: C.green, fontWeight: 700 }}>+55%</span>, <span style={{ color: C.red, fontWeight: 700 }}>−9.5%</span>, <span style={{ color: C.green, fontWeight: 700 }}>+21.5%</span>, <span style={{ color: C.green, fontWeight: 700 }}>+55.6%</span>. Only once (2018→2019) did back-to-back negatives occur — and what followed was explosive: 2020–2024 compounded at 26% CAGR.
          </div>
        </div>
        ...
        <div style={{ ...card, background: `${C.amber}0c`, border: `1px solid ${C.amber}33` }}>
          <div style={{ fontSize: 13.5, color: C.text2, lineHeight: 1.65 }}>
            <strong style={{ color: C.amber }}>Read the recovery paths with a pinch of salt.</strong> The 2027–2030 cascade uses the deck's post-drawdown analogues — Indian smallcaps <em>did</em> snap back violently after every crash, but 22 years is a short, mostly-bull sample. The P10 "unlucky" column is where a longer bear plays out. Use the <strong style={{ color: C.amber }}>forward-return haircut</strong> if you think the next cycle runs below history.
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ fontSize: 12.5, color: C.text3, marginBottom: 12 }}>Adjust the sliders to assign your probability to each outcome. Historical base rates (given 2025 was negative) shown as reference.</div>
          {SCENARIOS.map((s, i) => (
        ...
        <div style={card}>
          ...
          <div style={{ fontSize: 11.5, color: C.text4, marginTop: 12, lineHeight: 1.55 }}>{N_SIMS.toLocaleString('en-IN')} simulations sample a 2026 scenario from your weights and add {committed.vol}% annual volatility. The shaded chart band is the P10–P90 range.{haircut > 0 && <span style={{ color: C.amber }}> Forward haircut −{haircut}pp/yr applied.</span>}</div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ ...mono, fontSize: 11, color: C.text3, marginBottom: 8 }}>Nominal returns shown; <span style={{ color: C.cyan }}>real</span> = after −{INFLATION}% inflation.</div>
          <div style={{ ...mono, fontSize: 11, color: C.text3, marginBottom: 8, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <span><span style={{ color: C.gold }}>━</span> expected</span><span><span style={{ color: C.purple }}>▒</span> P10–P90</span><span><span style={{ color: C.purple }}>▓</span> P25–P75</span>{showBench && <span><span style={{ color: C.blue }}>┈</span> Nifty 50 median</span>}<span style={{ color: C.text4 }}>| bars = actual 2023–2025</span>
          </div>
          <svg viewBox={`0 0 ${chart.W} ${chart.H}`} style={{ width: '100%', height: 'auto', display: 'block' }} preserveAspectRatio="xMidYMid meet">
            ...
            <polygon points={fanPoly} fill={`${C.purple}1f`} /><polygon points={fanInner} fill={`${C.purple}30`} />
        ...
        {/* Wealth + risk */}
        <div style={card}>
          <div style={cardLabel}>{inrCompact(committed.capital)}{committed.mode === 'sip' ? ` + ${inrCompact(committed.monthlySip)}/mo SIP` : ''} invested Dec 2025 → projected growth {committed.realReturns ? '(real)' : '(nominal)'}</div>
          ...
          <div style={{ fontSize: 11, color: C.text3, textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 700, marginBottom: 10, ...mono }}>Monte-Carlo outcome range · {N_SIMS.toLocaleString('en-IN')} paths</div>
          ...
          {showBench && (<div style={{ fontSize: 12.5, color: C.text3, marginTop: 12, lineHeight: 1.55 }}>Smallcap median {inrCompact(mc.ending.p50)} vs Nifty 50 median {inrCompact(mc.benchMedianEnd)} — {mc.ending.p50 > mc.benchMedianEnd ? `the extra ${inrCompact(mc.ending.p50 - mc.benchMedianEnd)} is your reward for smallcap volatility. The P10 (${inrCompact(mc.ending.p10)}) is the price if the cycle turns.` : `the largecap median is higher — your bearish weights don't justify the extra smallcap risk.`}</div>)}
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ marginTop: 12, background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 10, padding: 15 }}>
            ...
            <div style={{ fontSize: 12.5, color: C.text2, lineHeight: 1.6 }}>2027–2030 are probability-weighted conditional paths — each 2026 scenario triggers a different trajectory based on what historically followed:
              ...
              <div style={{ marginTop: 10 }}>Expected 5Y CAGR of <strong style={{ color: C.cyan }}>{detCagr.toFixed(1)}%</strong> {Math.abs(detCagr - 14) < 4 ? 'sits near the long-run average (~14%).' : detCagr > 18 ? 'is above the long-run average — you may be overweighting bullish scenarios (or haircut too low).' : 'is below the long-run average — bearish or haircut-adjusted.'}</div>
            </div>
          </div>
        </div>
