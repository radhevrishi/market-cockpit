...
// zzz404/zzz406 — QQQ (Nasdaq-100) Scenario Simulator.
//
// Calibrated from the real 40-year Nasdaq-100 record (1986–2025). Data-driven
// buckets, default probabilities and cascade; parametric + historical-bootstrap
// Monte-Carlo; small-sample shrinkage; forward-return haircut.
//
// zzz406 fix: deterministic outputs (implied odds, expected returns, cascade,
// wealth path, insights) are FULLY LIVE with the sliders. Only the Monte-Carlo
// ranges (P10–P90 + risk panel) wait for the Refresh button.

import React, { useEffect, useMemo, useState, useCallback } from 'react';
import { NASDAQ100, computeStats, bucketize, freqProbs, conditionalPaths, type BucketDef, type Bucket } from '@/lib/market-history';
...
const PATHS = conditionalPaths(NASDAQ100, BUCKETS, 4, 4);
...
const N_SIMS = 4000;
const BENCH_MEAN = 10.5, BENCH_VOL = 15.5, US_INFLATION = 3.0;
...
interface MCInputs { probs: number[]; capital: number; mode: 'lumpsum' | 'sip'; monthlySip: number; realReturns: boolean; vol: number; haircut: number; engine: 'parametric' | 'bootstrap'; }
const DEF_MC: MCInputs = { probs: DEFAULT_PROBS, capital: 10000, mode: 'lumpsum', monthlySip: 500, realReturns: false, vol: 28, haircut: 0, engine: 'bootstrap' };
interface MCResult { band: { p10: number; p25: number; p50: number; p75: number; p90: number }[]; ending: { p10: number; p25: number; p50: number; p75: number; p90: number }; cagr: { p10: number; p50: number; p90: number }; pLoss: number; pDouble: number; pTriple: number; pHalf: number; invested: number; benchMedianEnd: number; }
...
export default function QqqSimulatorPage() {
  ...
  const [mcSnap, setMcSnap] = useState<MCInputs>(DEF_MC);

  useEffect(() => {
    try {
      const qs = new URLSearchParams(window.location.search);
      const snap: MCInputs = { ...DEF_MC };
      ...
      setRealReturns(snap.realReturns); setVol(snap.vol); setEngine(snap.engine); setHaircut(snap.haircut); setMcSnap(snap);
    } catch { /* ignore */ }
  }, []);
  ...
  const { expNom, expReal } = useMemo(() => {
    ...
    const nom = [e2026, eY(0), eY(1), eY(2), eY(3)].map(r => r - haircut);
    return { expNom: nom, expReal: nom.map(r => r - US_INFLATION) };
  }, [norm, haircut]);
  ...
  // ── MC (snapshot; Refresh commits) ──
  const live: MCInputs = { probs, capital, mode, monthlySip, realReturns, vol, haircut, engine };
  const mcStale = JSON.stringify(live) !== JSON.stringify(mcSnap);
  const run = useCallback(() => setMcSnap({ probs: probs.slice(), capital, mode, monthlySip, realReturns, vol, haircut, engine }), [probs, capital, mode, monthlySip, realReturns, vol, haircut, engine]);
  ...
  const mc = useMemo<MCResult>(() => {
    ...
    for (let s = 0; s < N_SIMS; s++) {
      ...
      if (mcSnap.engine === 'bootstrap' && b.years.length >= 2) {
        ...
        for (let h = 0; h < 4; h++) seq.push(clamp(path[h] + gaussian(rng) * mcSnap.vol, -60, 130));
      }
      const bseq: number[] = []; for (let y = 0; y < 5; y++) bseq.push(clamp(BENCH_MEAN + gaussian(rng) * BENCH_VOL, -45, 60));
      let w = mcSnap.capital, wb = mcSnap.capital;
      for (let y = 0; y < 5; y++) {
        const nomRet = seq[y] - mcSnap.haircut; yearRet[y].push(nomRet); benchRet[y].push(bseq[y]);
        const rS = mcSnap.realReturns ? nomRet - US_INFLATION : nomRet; const rB = mcSnap.realReturns ? bseq[y] - US_INFLATION : bseq[y];
    ...
    return { band, ending: { p10: pctile(soEnd, 10), p25: pctile(soEnd, 25), p50: pctile(soEnd, 50), p75: pctile(soEnd, 75), p90: pctile(soEnd, 90) }, cagr: { p10: pctile(cagrArr, 10), p50: pctile(cagrArr, 50), p90: pctile(cagrArr, 90) }, pLoss: (endW.filter(w => w < inv).length / endW.length) * 100, pDouble: (endW.filter(w => w >= inv * 2).length / endW.length) * 100, pTriple: (endW.filter(w => w >= inv * 3).length / endW.length) * 100, pHalf: (endW.filter(w => w <= inv * 0.5).length / endW.length) * 100, invested: inv, benchMedianEnd: pctile(soBench, 50) };
  }, [normMc, mcSnap]);
  ...
  const copyLink = useCallback(() => { try { const qs = new URLSearchParams(); qs.set('p', probs.join(',')); qs.set('cap', String(capital)); if (mode === 'sip') { qs.set('mode', 'sip'); qs.set('sip', String(monthlySip)); } if (realReturns) qs.set('real', '1'); qs.set('vol', String(vol)); qs.set('eng', engine); if (haircut) qs.set('hc', String(haircut)); navigator.clipboard?.writeText(`${window.location.origin}${window.location.pathname}?${qs.toString()}`); setSavedFlash('🔗 Link copied'); setTimeout(() => setSavedFlash(''), 2000); } catch { setSavedFlash('⚠ Could not copy'); } }, [probs, capital, mode, monthlySip, realReturns, vol, haircut, engine]);
  ...
  const chart = useMemo(() => {
    ...
    const cols = HIST_BARS.length + FORECAST_YEARS.length; const colW = (W - padL - padR) / cols; const xOf = (i: number) => padL + colW * (i + 0.5);
      ...
      <div style={{ maxWidth: 1360, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 18, padding: '18px 22px', background: `linear-gradient(135deg, ${C.cyan}18 0%, ${C.panel} 100%)`, border: `1px solid ${C.cyan}33`, borderRadius: 14, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ ...mono, fontSize: 13, color: C.text3, marginTop: 6 }}>Calibrated from 40 real years. Expected returns update live; Monte-Carlo ranges recompute on Refresh.</div>
          </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 10, marginBottom: 14 }}>
            ...
            ].map(t => (<div key={t.label} style={{ background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 8, padding: '11px 12px', textAlign: 'center' }}><div style={{ fontSize: 10.5, color: C.text3, marginBottom: 4 }}>{t.label}</div><div style={{ ...mono, fontSize: 16, fontWeight: 800, color: t.color }}>{t.value}</div></div>))}
          </div>
          <div style={{ fontSize: 13.5, color: C.text2, lineHeight: 1.65 }}>The Nasdaq-100 is a two-regime beast: it compounded at <span style={{ color: C.green, fontWeight: 700 }}>~{STATS.cagr.toFixed(0)}%</span> for four decades, yet lived through the 2000–2002 dot-com bust, a −41.9% 2008, and a −33.0% 2022. Recoveries were violent — +49% in 2003, +53% in 2009, +54% in 2023.</div>
        </div>

        <div style={{ ...card, background: `${C.amber}0c`, border: `1px solid ${C.amber}33` }}>
          <div style={{ fontSize: 13.5, color: C.text2, lineHeight: 1.65 }}>
            <strong style={{ color: C.amber }}>Is this too optimistic?</strong> Partly. This is calibrated on the <strong>best-performing major index of the last 40 years</strong>, which recovered from every drawdown — so even bearish 2026 inputs cascade back to a healthy CAGR. That's real but regime-dependent: a 2000–2012 lost decade shows up in your <strong style={{ color: C.red }}>P10 column</strong>, not the median. Thin scenarios are shrunk toward the mean so one lucky year can't inflate the path, and the <strong style={{ color: C.amber }}>forward-return haircut</strong> lets you dial the future down.
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
            {BUCKETS.map(b => (
              <div key={b.id} style={{ background: C.panel2, border: `1px solid ${b.color}22`, borderRadius: 10, padding: '11px 12px' }}>
                ...
                <div style={{ height: 8, background: C.bg, borderRadius: 4, overflow: 'hidden', marginBottom: 6 }}><div style={{ width: `${(b.count / maxHistBucket) * 100}%`, height: '100%', background: b.color, borderRadius: 4 }} /></div>
        ...
        <div style={card}>
          ...
          <div style={{ fontSize: 12.5, color: C.text3, marginBottom: 12 }}>Defaults are the <strong style={{ color: C.text2 }}>real historical frequencies</strong> — your starting point is what the Nasdaq-100 actually did.</div>
          {BUCKETS.map((s, i) => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 0', borderBottom: i < BUCKETS.length - 1 ? `1px solid ${C.border}` : 'none', flexWrap: 'wrap' }}>
              <div style={{ minWidth: 160 }}><span style={{ color: s.color, fontWeight: 700, fontSize: 14 }}>{s.label}</span><div style={{ fontSize: 11, color: C.text3 }}>{s.range}</div></div>
              <div style={{ minWidth: 70, textAlign: 'center' }}><span style={{ ...mono, fontSize: 10.5, padding: '2px 8px', borderRadius: 5, background: `${s.color}15`, color: s.color, border: `1px solid ${s.color}30` }}>hist: {s.freq.toFixed(0)}%</span></div>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 12, minWidth: 240 }}><input type="range" min={0} max={80} value={probs[i]} onChange={e => setProb(i, parseInt(e.target.value, 10))} style={{ flex: 1, accentColor: s.color, cursor: 'pointer' }} /><div style={{ ...mono, minWidth: 48, textAlign: 'right', color: s.color, fontWeight: 700, fontSize: 15 }}>{probs[i]}%</div></div>
            </div>
          ))}
          <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
            <span style={{ ...mono, fontSize: 14, fontWeight: 700, color: total === 100 ? C.green : total > 100 ? C.red : C.amber }}>Total: {total}% {total === 100 ? '✓' : total < 100 ? `(need ${100 - total}%)` : `(${total - 100}% over)`}{total !== 100 && <span style={{ color: C.text3, fontWeight: 400 }}> — auto-normalised</span>}</span>
            <span style={{ ...mono, fontSize: 13, color: C.text2 }}>→ live: <strong style={{ color: expNom[0] >= 0 ? C.green : C.red }}>E[2026] {expNom[0] >= 0 ? '+' : ''}{expNom[0].toFixed(1)}%</strong> · <span style={{ color: pNeg > histNegPct + 10 ? C.red : C.amber }}>P(down) {pNeg.toFixed(0)}%</span> <span style={{ color: C.text4 }}>vs {histNegPct.toFixed(0)}% hist</span></span>
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: 'flex', gap: 22, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <label style={{ fontSize: 12.5, color: C.text3 }}>{mode === 'sip' ? 'Initial lumpsum ($)' : 'Amount invested ($)'}<input type="number" min={100} step={1000} value={capital} onChange={e => setCapital(clamp(parseInt(e.target.value, 10) || 0, 0, 1e11))} style={{ display: 'block', marginTop: 5, width: 150, background: C.panel2, border: `1px solid ${C.border}`, color: C.text, borderRadius: 7, padding: '8px 11px', ...mono, fontSize: 14 }} /></label>
            <div><div style={{ fontSize: 12.5, color: C.text3, marginBottom: 5 }}>Mode</div><div style={{ display: 'flex', gap: 6 }}><button onClick={() => setMode('lumpsum')} style={chip(mode === 'lumpsum', C.cyan)}>Lumpsum</button><button onClick={() => setMode('sip')} style={chip(mode === 'sip', C.cyan)}>Monthly DCA</button></div></div>
            {mode === 'sip' && (<label style={{ fontSize: 12.5, color: C.text3 }}>Monthly ($)<input type="number" min={10} step={100} value={monthlySip} onChange={e => setMonthlySip(clamp(parseInt(e.target.value, 10) || 0, 0, 1e9))} style={{ display: 'block', marginTop: 5, width: 120, background: C.panel2, border: `1px solid ${C.border}`, color: C.text, borderRadius: 7, padding: '8px 11px', ...mono, fontSize: 14 }} /></label>)}
            <div><div style={{ fontSize: 12.5, color: C.text3, marginBottom: 5 }}>MC engine</div><div style={{ display: 'flex', gap: 6 }}><button onClick={() => setEngine('bootstrap')} style={chip(engine === 'bootstrap', C.purple)}>Historical bootstrap</button><button onClick={() => setEngine('parametric')} style={chip(engine === 'parametric', C.purple)}>Parametric</button></div></div>
            <div><div style={{ fontSize: 12.5, color: C.text3, marginBottom: 5 }}>Wealth basis</div><button onClick={() => setRealReturns(v => !v)} style={chip(realReturns, C.amber)}>{realReturns ? `Real (−${US_INFLATION}%)` : 'Nominal'}</button></div>
            <div><div style={{ fontSize: 12.5, color: C.text3, marginBottom: 5 }}>Benchmark</div><button onClick={() => setShowBench(v => !v)} style={chip(showBench, C.blue)}>{showBench ? '✓ S&P 500' : 'S&P 500'}</button></div>
            ...
            {engine === 'parametric' && (<label style={{ fontSize: 12.5, color: C.text3 }}>Volatility: <span style={{ ...mono, color: C.text2 }}>{vol}%</span><input type="range" min={15} max={40} value={vol} onChange={e => setVol(parseInt(e.target.value, 10))} style={{ display: 'block', marginTop: 7, width: 150, accentColor: C.purple, cursor: 'pointer' }} /></label>)}
          </div>
        </div>

        {/* Step 2 — LIVE cards + chart */}
        <div style={card}>
          ...
          <div style={{ ...mono, fontSize: 11, color: C.text3, marginBottom: 8 }}>Nominal returns; <span style={{ color: C.cyan }}>real</span> = after −{US_INFLATION}% inflation. Shaded band = Monte-Carlo P10–P90 {mcStale ? <span style={{ color: C.amber }}>(stale — Refresh)</span> : '(current)'}.</div>
          <svg viewBox={`0 0 ${chart.W} ${chart.H}`} style={{ width: '100%', height: 'auto', display: 'block' }} preserveAspectRatio="xMidYMid meet">
            {[-60, -40, -20, 20, 40, 60].filter(v => Math.abs(v) <= chart.maxAbs).map(v => (<g key={v}><line x1={chart.padL} y1={chart.yOf(v)} x2={chart.W - chart.padR} y2={chart.yOf(v)} stroke={C.border} strokeWidth={0.5} strokeDasharray="2 4" /><text x={chart.W - chart.padR} y={chart.yOf(v) - 2} fill={C.text4} fontSize={9} textAnchor="end" style={mono}>{v > 0 ? '+' : ''}{v}%</text></g>))}
        ...
        {/* Wealth (live) */}
        <div style={card}>
          <div style={cardLabel}>{usd(capital)}{mode === 'sip' ? ` + ${usd(monthlySip)}/mo DCA` : ''} invested Dec 2025 → projected growth {realReturns ? '(real)' : '(nominal)'} · live</div>
          {wealthPath.map((v, i) => { const maxVal = Math.max(...wealthPath.map(p => p.val)); const pct = (v.val / maxVal) * 100; const up = v.val >= invested; const col = up ? C.green : C.red; return (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 7 }}><div style={{ ...mono, fontSize: 12, color: C.text3, minWidth: 74 }}>{v.yr}</div><div style={{ flex: 1, background: C.panel2, borderRadius: 6, height: 26, position: 'relative', overflow: 'hidden' }}><div style={{ width: `${pct}%`, height: '100%', background: `${col}25`, border: `1px solid ${col}45`, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: 10, minWidth: 100 }}><span style={{ ...mono, fontSize: 12, color: col, fontWeight: 700 }}>{usdFull(v.val)}</span></div></div></div>); })}
          <div style={{ ...mono, fontSize: 13.5, marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.border}` }}><span style={{ color: C.cyan, fontWeight: 700 }}>{mode === 'sip' ? 'Simple growth (on contributions)' : '5Y CAGR (expected)'}: {detCagr.toFixed(1)}%</span><span style={{ color: C.text3 }}> · Total return: {detTotalRet >= 0 ? '+' : ''}{detTotalRet.toFixed(0)}%</span><span style={{ color: C.text3 }}> · {usd(invested)} → {usd(finalDet)}</span></div>
          {mode === 'sip' && <div style={{ ...mono, fontSize: 11, color: C.text4, marginTop: 6 }}>Note: for DCA this is simple growth on total contributions (all money treated as invested at t0), not a true time-weighted CAGR — it understates the real per-dollar return.</div>}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 16, padding: '12px 16px', background: mcStale ? `${C.amber}12` : `${C.green}0c`, border: `1px solid ${mcStale ? C.amber : C.green}44`, borderRadius: 12 }}>
          <button onClick={run} disabled={!mcStale} style={{ ...chip(false, mcStale ? C.amber : C.green), fontSize: 13.5, padding: '9px 18px', opacity: mcStale ? 1 : 0.6, cursor: mcStale ? 'pointer' : 'default' }}>⟳ {mcStale ? 'Refresh Monte-Carlo ranges' : 'Ranges up to date'}</button>
          <span style={{ ...mono, fontSize: 12, color: mcStale ? C.amber : C.text3 }}>{mcStale ? 'The bands & risk panel below reflect your PREVIOUS inputs. Click to re-run 4,000 simulations.' : 'Ranges match your current inputs.'}</span>
        </div>

        <div style={{ ...card, opacity: mcStale ? 0.6 : 1 }}>
          <div style={cardLabel}>Monte-Carlo outcome range · {N_SIMS.toLocaleString('en-US')} {mcSnap.engine} paths {mcStale ? '· ⟳ stale' : ''}</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: 12 }}>
            {[{ label: 'P10 · unlucky', val: usd(mc.ending.p10), color: C.red }, { label: 'P25', val: usd(mc.ending.p25), color: C.amber }, { label: 'P50 · median', val: usd(mc.ending.p50), color: C.text }, { label: 'P75', val: usd(mc.ending.p75), color: C.green }, { label: 'P90 · lucky', val: usd(mc.ending.p90), color: C.green }].map(t => (<div key={t.label} style={{ background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 8, padding: '11px 12px', textAlign: 'center' }}><div style={{ fontSize: 10, color: C.text3, marginBottom: 4, ...mono }}>{t.label}</div><div style={{ ...mono, fontSize: 15, fontWeight: 800, color: t.color }}>{t.val}</div></div>))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10 }}>
            {[{ label: 'P(loses money)', val: `${mc.pLoss.toFixed(0)}%`, color: mc.pLoss > 25 ? C.red : C.amber }, { label: 'P(doubles+)', val: `${mc.pDouble.toFixed(0)}%`, color: C.green }, { label: 'P(triples+)', val: `${mc.pTriple.toFixed(0)}%`, color: C.green }, { label: 'P(halves)', val: `${mc.pHalf.toFixed(0)}%`, color: mc.pHalf > 10 ? C.red : C.text2 }, { label: mcSnap.mode === 'sip' ? 'Simple growth P10 → P90' : 'CAGR P10 → P90', val: `${mc.cagr.p10.toFixed(0)}% → ${mc.cagr.p90.toFixed(0)}%`, color: C.cyan }, ...(showBench ? [{ label: 'S&P 500 median end', val: usd(mc.benchMedianEnd), color: C.blue }] : [])].map(t => (<div key={t.label} style={{ background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 8, padding: '11px 12px', textAlign: 'center' }}><div style={{ fontSize: 10, color: C.text3, marginBottom: 4, ...mono }}>{t.label}</div><div style={{ ...mono, fontSize: 14.5, fontWeight: 800, color: t.color }}>{t.val}</div></div>))}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 10, padding: 15 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 10, fontSize: 13.5, color: C.text2 }}>
              <div>P(2026 down year): <strong style={{ color: pNeg > histNegPct + 10 ? C.red : C.amber }}>{pNeg.toFixed(0)}%</strong> <span style={{ color: C.text4 }}>vs {histNegPct.toFixed(0)}% hist</span></div>
              <div>P(2026 boom +40%): <strong style={{ color: C.green }}>{pBoom.toFixed(0)}%</strong> <span style={{ color: C.text4 }}>vs {BUCKETS[5].freq.toFixed(0)}% hist</span></div>
              ...
              <div>Your 5Y {mode === 'sip' ? 'simple growth' : 'CAGR'}: <strong style={{ color: C.cyan }}>{detCagr.toFixed(1)}%</strong> <span style={{ color: C.text4 }}>vs +{STATS.cagr.toFixed(0)}% actual</span></div>
            </div>
          </div>
          {pNeg > histNegPct + 12 && (<div style={{ marginTop: 10, padding: '11px 13px', background: `${C.red}0e`, border: `1px solid ${C.red}22`, borderRadius: 8, fontSize: 12.5, color: C.text2, lineHeight: 1.55 }}><strong style={{ color: C.red }}>Bearish vs history:</strong> you're pricing {pNeg.toFixed(0)}% odds of a down 2026, above the {histNegPct.toFixed(0)}% across 40 years. Every down year snapped back +50%+ within a couple years — if you doubt that repeats, raise the forward haircut.</div>)}
          {pBoom > 35 && (<div style={{ marginTop: 10, padding: '11px 13px', background: `${C.green}0e`, border: `1px solid ${C.green}22`, borderRadius: 8, fontSize: 12.5, color: C.text2, lineHeight: 1.55 }}><strong style={{ color: C.green }}>Aggressive bull:</strong> {pBoom.toFixed(0)}% odds on a +40% boom is {(pBoom / Math.max(BUCKETS[5].freq, 1)).toFixed(1)}× the historical base rate. Back-to-back booms are rare; the cascade pulls 2027+ toward mean reversion.</div>)}
          <div style={{ marginTop: 12, background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 10, padding: 15 }}>
            ...
            <div style={{ fontSize: 12.5, color: C.text2, lineHeight: 1.6 }}>For each 2026 bucket, 2027–2030 are the <strong>actual average of what followed</strong>, shrunk toward the mean by sample size:
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 6, marginTop: 10, ...mono, fontSize: 11 }}>
                {BUCKETS.map(b => (<div key={b.id} style={{ color: b.color }}>If {b.label.toLowerCase()} → {PATHS[b.id].map(v => `${v >= 0 ? '+' : ''}${v.toFixed(0)}`).join(', ')} <span style={{ color: C.text4 }}>(n={b.count}{b.count < 3 ? ' · shrunk' : ''})</span></div>))}
              </div>
              <div style={{ marginTop: 10 }}>Expected 5Y {mode === 'sip' ? 'simple growth' : 'CAGR'} of <strong style={{ color: C.cyan }}>{detCagr.toFixed(1)}%</strong> {Math.abs(detCagr - STATS.cagr) < 4 ? `sits near the 40-year actual (~${STATS.cagr.toFixed(0)}%).` : detCagr > STATS.cagr ? 'is above the 40-year actual — bullish or haircut too low.' : 'is below the 40-year actual — bearish or haircut-adjusted.'}</div>
            </div>
          </div>
        </div>

        <div style={{ background: `${C.amber}0c`, border: `1px solid ${C.amber}28`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
          <div style={{ fontSize: 12.5, color: C.text2, lineHeight: 1.6 }}><strong style={{ color: C.amber }}>⚠ Important.</strong> Projections extrapolate from 40 years of Nasdaq-100 history. Heavily concentrated in mega-cap tech; future regimes may not rhyme with the past. Annual std deviation ~{STATS.stdev.toFixed(0)}%. Analytical exercise, <strong>not investment advice</strong>.</div>
        </div>
