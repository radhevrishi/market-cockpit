    ...
    <div style={{ background: C.bg, color: C.text, minHeight: '100vh', padding: '26px 24px' }}>
      <div style={{ maxWidth: 1360, margin: '0 auto' }}>
        ...
        <div style={card}>
          ...
          <div style={{ ...mono, fontSize: 13.5, marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.border}` }}><span style={{ color: C.cyan, fontWeight: 700 }}>5Y CAGR (expected): {detCagr.toFixed(1)}%</span><span style={{ color: C.text3 }}> · Total return: {detTotalRet >= 0 ? '+' : ''}{detTotalRet.toFixed(0)}%</span><span style={{ color: C.text3 }}> · {usd(invested)} → {usd(finalDet)}</span></div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ marginTop: 12, background: C.panel2, border: `1px solid ${C.border}`, borderRadius: 10, padding: 15 }}>
            ...
            <div style={{ fontSize: 12.5, color: C.text2, lineHeight: 1.6 }}>For each 2026 bucket, 2027–2030 are the <strong>actual average of what followed</strong>, shrunk toward the mean by sample size:
              ...
              <div style={{ marginTop: 10 }}>Expected 5Y CAGR of <strong style={{ color: C.cyan }}>{detCagr.toFixed(1)}%</strong> {Math.abs(detCagr - STATS.cagr) < 4 ? `sits near the 40-year actual (~${STATS.cagr.toFixed(0)}%).` : detCagr > STATS.cagr ? 'is above the 40-year actual — bullish or haircut too low.' : 'is below the 40-year actual — bearish or haircut-adjusted.'}</div>
            </div>
          </div>
        </div>
