...
// ═══════════════════════════════════════════════════════════════════════════
// STRATEGIC HIRES TRACKER (TheWrap Module 3)
//
// Surfaces CXO / senior-management appointments where the incoming executive
// arrives from a tier-1 employer (Google/Goldman/McKinsey/TCS/Reliance/…).
// Runs detectStrategicHire() from lib/thewrap-detectors over the dual-source
// news + NSE/BSE filings feed. See lib/thewrap-feed.ts for the shared pipeline.
// ═══════════════════════════════════════════════════════════════════════════

import { useEffect, useMemo, useState } from 'react';
import { detectStrategicHire } from '@/lib/thewrap-detectors';
import {
  fetchWrapFeed,
  runDetector,
  relativeTime,
...
const PAGE_TITLE = 'Strategic Hires';
const PAGE_EMOJI = '👔';
const PAGE_DESC =
  'CXO & senior appointments where the incoming executive arrives from a tier-1 employer — a leadership-upgrade re-rating catalyst the standard news feed buries.';
const FACET_KEY = 'employer';
const FACET_LABEL = 'Employer';
const EMPTY_THING = 'strategic hires';

export default function StrategicHiresPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const found = runDetector(res.items, detectStrategicHire);
      setMatches(found);
  ...
  // Facet values from the detector's meta.
    ...
    <div style={{ padding: '20px 24px', maxWidth: 1180, margin: '0 auto', color: '#E2E8F0' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 18, flexWrap: 'wrap' }}>
        <input
          ...
          placeholder="Search ticker, company, employer…"
          style={inputStyle}
        />
...
// ── Shared sub-components (local to this page file) ───────────────────────────
              ...
              Object.entries(m.signal.meta)
                ...
                .map(([k, v]) => (
                  <span
                    key={k}
                    style={{
                      fontSize: 10, color: k === facetKey ? '#A78BFA' : '#94A3B8',
  ...
  return (
    <button
      ...
      style={{
        ...
        borderColor: active ? '#A78BFA' : '#1E2D45',
