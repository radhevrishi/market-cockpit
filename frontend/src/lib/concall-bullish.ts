...
export type ConcallFilingType =
  ...
  | 'ORDER_RECEIPT'
  | 'RATING_ACTION'
  // PATCH 0713 — event-driven filings the prior classifier dropped to
  // null (silently). Each is a distinct re-rating catalyst:
  //   CAPEX_ANNOUNCE   — board-approved capex / new plant / brownfield
  //   BUYBACK_ANNOUNCE — buyback offer / tender / open-market
  //   PROMOTER_TXN     — promoter buy / sell / inter-se / creeping
  //   BLOCK_DEAL       — bulk / block window / 5% off-market
  //   MA_EVENT         — scheme of amalgamation / demerger / slump sale
  //   GOVERNANCE       — KMP appointment / auditor change / board
  //   USFDA_EVENT      — pharma marketing-auth / Form 483 / EIR
  | 'CAPEX_ANNOUNCE'
  | 'BUYBACK_ANNOUNCE'
  | 'PROMOTER_TXN'
  | 'BLOCK_DEAL'
...
const RELEVANCE_PATTERNS: Array<{ type: ConcallFilingType; re: RegExp }> = [
  ...
  { type: 'ORDER_RECEIPT', re: /bagging\s*\/\s*receiving|receipt\s+of\s+order|letter\s+of\s+(?:award|acceptance|intent)|\bLoA\b|\bLoI\b|\bNOA\b|notice\s+of\s+award|\bMOU\b|memorandum\s+of\s+understanding|work\s+order|purchase\s+order|quarterly\s+POs?|contract\s+award|order\s+received|receiving\s+of\s+orders|bagging\s+of\s+orders|bagged\s+(?:an?\s+|the\s+|new\s+|large\s+|big[- ]?ticket\s+|landmark\s+|maiden\s+|repeat\s+)?(?:order|contract|project)|wins?\s+(?:an?\s+|the\s+|new\s+|large\s+|big[- ]?ticket\s+|landmark\s+|maiden\s+|repeat\s+)?(?:order|contract|project|deal|mandate|tender|bid)|secured\s+(?:an?\s+|the\s+|new\s+|large\s+|big[- ]?ticket\s+|landmark\s+|maiden\s+|repeat\s+)?(?:order|contract|project|mandate)|order\s+(?:intake|book\s+update|win)|\bnew\s+order\b|\brepeat\s+order\b|\bfollow[- ]?on\s+order\b|\bL1\s+(?:bidder|status|position|in\s+the|for\s+the)|emerged\s+(?:as\s+)?(?:the\s+)?(?:L1|lowest|preferred|successful)\s*bidder|selected\s+(?:as\s+(?:the\s+)?)?(?:lowest|preferred|successful)?\s*bidder|framework\s+(?:agreement|contract)|rate\s+contract|\bEPC\s+(?:contract|order|project|award)|supply\s+(?:agreement|contract)|turnkey\s+(?:project|contract|order)|design\s+&\s+build|\bEPCM\b|\bBOT\b\s+(?:contract|order|project)|\bBOOT\b\s+(?:contract|order|project)|\bHAM\s+(?:project|contract)|\bAMC\s+(?:contract|order)|annual\s+maintenance\s+contract|\bO&M\s+(?:contract|agreement)|operations?\s+(?:and|&)\s+maintenance|strategic\s+(?:agreement|partnership|contract)\b.{0,40}(?:signed|entered|inked)|definitive\s+agreement|(?:awarded|won)\s+(?:a\s+|the\s+)?(?:tender|bid|RFP|RFQ|EPC|contract)|binding\s+(?:offer|agreement)|(?:DDC|DPSU)\s+(?:award|contract|order)|Rail(?:way)?s?\s+(?:order|contract|award)|(?:exclusive|sole)\s+(?:supplier|distributor|vendor)\s+(?:agreement|contract)|(?:multi[- ]?year|long[- ]?term)\s+supply\s+agreement|emergency\s+(?:purchase|supply)\s+order|deal\s+(?:wins?|signing|inked|secured|closed)/i }, // PATCH 0713 widened
  ...
  // PATCH 0713 — new RELEVANCE categories. Previously these slipped past
  // classifyFiling and got silently dropped. Each represents a distinct
  // event-driven catalyst that the existing /news + /order-book +
  // /rating-actions stream should surface.
  { type: 'CAPEX_ANNOUNCE', re: /capex\s+(?:plan|announce|approval|sanction)|capital\s+expenditure\s+(?:announce|plan|approval|sanction)|board\s+(?:approves?|approval)\s+(?:capex|capital\s+expenditure|new\s+plant|expansion)|new\s+(?:plant|facility|line)\s+(?:announce|commission|inaugurat|ground[- ]?breaking)|brownfield\s+(?:expansion|capex|approval)|greenfield\s+(?:expansion|capex|approval|project)|phase\s+(?:1|2|3|I|II|III)\s+(?:commission|complet)|commercial\s+production\s+(?:from|begins?|to\s+begin|commenced)|trial\s+production\s+(?:begins?|commenced|started)|capacity\s+(?:expansion|addition|augmentation|enhancement|debottlenecking)|ramp\s+to\s+nameplate|first\s+commercial\s+production|FCD\b|RFCD\b|go[- ]live\s+target|DDP\s+(?:approval|sanction)/i }, // PATCH 0713
  { type: 'BUYBACK_ANNOUNCE', re: /\bbuy[- ]?back\b|\bshare\s+repurchase\b|repurchase\s+of\s+(?:equity\s+)?shares|board\s+(?:approves?|considers?)\s+buy[- ]?back|buy[- ]?back\s+(?:through\s+)?tender|open[- ]?market\s+buy[- ]?back|buy[- ]?back\s+(?:offer|price|record\s+date|window|tendering)|EGM\s+for\s+buy[- ]?back|extraordinary\s+general\s+meeting\s+for\s+buy[- ]?back/i }, // PATCH 0713
  { type: 'PROMOTER_TXN', re: /\b(?:disclosure\s+under\s+)?(?:SAST|PIT)\s+regulations?\b|\bpromoter\s+(?:open[- ]?market\s+)?(?:purchase|buy|sale|sell|offload|stake\s+(?:hike|increase|reduction|sale))\b|inter[- ]?se\s+transfer|creeping\s+acquisition|substantial\s+acquisition\s+of\s+shares|promoter\s+(?:reclassification|group\s+transfer|inter\s+se)|insider\s+(?:buy|purchase|sell|sale|trading\s+disclosure)|pledge\s+(?:invok|release|reduction|reduced|created|of\s+\d)|promoter\s+pledge\s+(?:revised|disclosure|update)|de[- ]?pledged?/i }, // PATCH 0713
  { type: 'BLOCK_DEAL', re: /\bblock\s+(?:deal|window|trade)\b|\bbulk\s+deal\b|\boff[- ]?market\s+(?:sale|transfer|deal)\b|institutional\s+block|large\s+trade\s+(?:executed|reported)|non[- ]?disposal\s+undertaking|NDU\s+(?:executed|signed)|5\s*%\s+(?:open[- ]?market|stake)\s+(?:sale|purchase)/i }, // PATCH 0713
  { type: 'MA_EVENT', re: /\bscheme\s+of\s+(?:amalgamation|arrangement|merger|demerger|capital\s+reduction)\b|\bcomposite\s+scheme\s+of\s+arrangement\b|\bdemerger\s+of\b|\bslump\s+sale\b|\bslump\s+exchange\b|\basset\s+purchase\s+agreement\b|\bshare\s+(?:swap|exchange)\s+ratio\b|\bmerger\s+ratio\b|\bcourt\s+convened\s+meeting\b|\bNCLT\s+(?:approval|order|sanction|hearing|petition)\b|\bbinding\s+term\s+sheet\b|\bhive[- ]?off\b|\bdivestment\s+(?:of|announce)/i }, // PATCH 0713
  { type: 'GOVERNANCE', re: /\b(?:appointment|cessation|resignation|change\s+in|reappointment)\s+of\s+(?:director|managing\s+director|MD|whole[- ]time\s+director|WTD|CEO|CFO|COO|CTO|CHRO|chief|key\s+managerial\s+personnel|KMP|company\s+secretary|CS|chairman|independent\s+director|auditor|statutory\s+auditor|secretarial\s+auditor)\b|\bauditor\s+(?:resigned|change|appointed)|\binduction\s+(?:in)?to\s+the\s+board\b|\bchange\s+in\s+(?:directorate|management|board\s+composition|control)\b/i }, // PATCH 0713
  { type: 'USFDA_EVENT', re: /\bUSFDA\b[^.]{0,80}(?:approval|inspection|EIR|VAI|OAI|Form\s+483|warning\s+letter|untitled\s+letter)|\bANDA\b\s*(?:approval|tentative|filing|grant|denial)|\bEstablishment\s+Inspection\s+Report\b|\bForm\s+483\b|\bMHRA\b[^.]{0,40}(?:approval|inspection|certificat)|\bWHO[- ]?GMP\b|\bCEP\b\s+(?:granted|issued|approval|filed)|\bCertificate\s+of\s+Suitability\b|\bMarketing\s+Authori[sz]ation\b|\bDCGI\b[^.]{0,40}(?:approval|approves?)|\btech(?:nology)?\s+transfer\b|\bTGA\b\s+(?:approval|listing)|\bPMDA\b\s+(?:approval)|\bsupplementary\s+NDA\b|\bsNDA\b|\bcomplete\s+response\s+letter\b|\bCRL\b|\bPAI\s+clearance\b|\bpre[- ]?approval\s+inspection\b|\bbiosimilar\s+(?:approv|launch|filing)\b|\blabel\s+expansion\b|\bextension\s+of\s+indication\b|\bfirst[- ]to[- ]file\b|\bPara\s+IV\b|\bDMF\s+(?:filed|submission|grant)\b/i }, // PATCH 0713
];
